    ...
    <div className="space-y-6">
      ...
      <div className="glass-card p-6 space-y-4">
        ...
        <textarea
          ...
          className="w-full h-32 px-3 py-2 bg-white border border-gray-200 rounded-lg text-base text-gray-700 font-mono resize-none focus:outline-none focus:border-blue-400"
        />
        <div className="flex items-center gap-3">
          <button onClick={handleImport}
            className="px-4 py-2 bg-purple-600/40 hover:bg-purple-600/60 rounded-lg text-base flex items-center gap-1.5 transition-colors">
            <ClipboardPaste size={14} /> 导入
          </button>
          {status && <span className="text-base text-gray-600">{status}</span>}
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {([
            ...
            <button key={type} onClick={() => handleExport(type)}
              className="p-3 bg-white hover:bg-gray-100/50 border border-gray-200 rounded-lg text-base text-gray-700 flex items-center gap-2 transition-colors">
