  ...
  return (
    <div className="glass-card p-6 space-y-4 mt-4">
      <div className="flex items-center justify-between">
        <h4 className="text-base font-semibold text-blue-700 flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-gray-500">{answer ? `${answer.length} 字` : '未填写'}</span>
        </div>
      </div>

      <textarea value={answer} onChange={(e) => setAnswer(e.target.value)}
        placeholder="用最简单的话写下你的理解..."
        className="w-full h-48 bg-white border border-gray-200 rounded-xl p-4 text-base text-gray-800 placeholder:text-gray-400 resize-y focus:outline-none focus:border-blue-400 transition-colors"
      />

      <div className="flex gap-2">
        <button onClick={() => persist()}
          className="px-4 py-2 bg-purple-600/50 hover:bg-purple-600/70 border border-blue-200 rounded-lg text-base text-blue-600 transition-colors"
        >{saved ? '✓ 已保存' : '保存'}</button>
        <button onClick={() => setShowRef(!showRef)}
          className={`px-4 py-2 border rounded-lg text-base transition-colors flex items-center gap-1 ${
        ...
        <button onClick={handleClear}
          className="px-4 py-2 bg-white hover:bg-gray-100/50 border border-gray-200 rounded-lg text-base text-gray-600 transition-colors"
        >清除</button>
      </div>

      {showRef && (
        <div className="bg-amber-500/5 border border-amber-500/20 rounded-xl p-4 space-y-2">
          <h5 className="text-sm font-semibold text-amber-700 flex items-center gap-1"><Sparkles size={12} /> 参考答案要点</h5>
      ...
      <div className="border-t border-gray-200 pt-4">
        <p className="text-sm text-gray-600 mb-2">对照参考后，给自己打分：</p>
        <div className="flex gap-2">
          {[
